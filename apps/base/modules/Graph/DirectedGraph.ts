...
export class DirectedGraph {
  ...
  public execute(data: any) {
    ...
    if (startNodes.length == 1) {
      startNodes[0].execute(data, process.env.CLUSTER_TOKEN)
    } else {
      // TODO: Make each start node take in whole payload?
      Lodash.each(startNodes, (node, i) => node.execute(data[i], process.env.CLUSTER_TOKEN));
    }

    // Continue execution by calling execute for remaining nodes
    // These will wait for parent execution before executing
    Lodash.each(other, (node) => node.execute({}, process.env.CLUSTER_TOKEN));
